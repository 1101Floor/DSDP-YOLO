...
class DySnakeConv(nn.Module):
    def __init__(self, inc, ouc, k=3, s=1, p=1, g=1, act=True):
        super().__init__()
        # 定义蛇形卷积的偏移量和卷积层
        self.conv = nn.Conv2d(inc, ouc, kernel_size=k, stride=s, padding=p, groups=g, bias=False)
...
# YAML 里使用的 C2f_HDSB
class C2f_HDSB(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = Conv(c1, c2, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)
        # 关键点：将原本的 Bottleneck 替换为包含蛇形卷积的结构
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class DPMA(nn.Module):
    """
    Dual-Path Multi-scale Attention (DPMA)
    通过双路径（通道+空间）融合，专为提取细长、不规则或边缘模糊目标（如油污）特征而设计
    """

    def __init__(self, c1, c2):
        super().__init__()
        # 用于通道数不对齐时的过渡
        self.cv = nn.Identity() if c1 == c2 else nn.Conv2d(c1, c2, 1, 1, bias=False)

        # 路径 1: 通道注意力 (Channel Path)
        mid_c = max(8, c1 // 16)
    ...
    def forward(self, x):
        ...
        # 2. 经过空间注意力机制 (融合平均池化和最大池化的空间特征)
        avg_spatial = torch.mean(x_c, dim=1, keepdim=True)
        max_spatial, _ = torch.max(x_c, dim=1, keepdim=True)
        s_attn = self.spatial_attn(torch.cat([avg_spatial, max_spatial], dim=1))

        # 3. 最终输出
        return self.cv(x_c * s_attn)
